    ...
    <div className="bg-background text-on-surface antialiased min-h-screen flex flex-col overflow-hidden relative font-body">
      ...
      <nav className="relative z-50 w-full px-8 py-6 flex justify-between items-center backdrop-blur-md border-b border-surface-variant/30">
        ...
        <div className="flex items-center gap-6">
          ...
          <Link href="/register" className="bg-surface-container-highest text-on-surface hover:bg-surface-container hover:text-primary rounded-md px-6 py-2.5 text-sm font-bold transition-all shadow-sm">Sign Up Free</Link>
        </div>
      </nav>
      ...
      <main className="relative z-10 flex-col flex items-center justify-center flex-grow px-4 text-center mt-12 mb-20 max-w-5xl mx-auto">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-md bg-surface-container-lowest border border-outline-variant/20 shadow-sm mb-8 animate-fade-in-up">
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-center justify-center w-full sm:w-auto">
          <Link href="/register" className="w-full sm:w-auto bg-gradient-to-br from-primary to-primary-container text-white rounded-md px-10 py-4 text-lg font-bold shadow-[0_8px_30px_rgb(74,75,215,0.3)] hover:scale-105 transition-all active:scale-95 flex items-center justify-center gap-2">
            Get Started <span className="material-symbols-outlined text-[20px]">arrow_forward</span>
          </Link>
          <Link href="/tracker" className="w-full sm:w-auto bg-surface-container-lowest text-on-surface border border-outline-variant/30 rounded-md px-10 py-4 text-lg font-bold shadow-sm hover:bg-surface transition-all flex items-center justify-center gap-2">
            View Live Tracker
          </Link>
        </div>
        
        {/* Floating Dashboard Preview Image placeholder based on User's Mac UI prompt */}
        <div className="mt-20 w-full rounded-md bg-surface-container-lowest p-2 shadow-2xl border border-surface-variant/50 ambient-shadow relative overflow-hidden group">
            <div className="absolute inset-0 bg-gradient-to-t from-background via-transparent to-transparent z-10 bottom-[-10px]"></div>
            <div className="bg-surface-container-low rounded-md w-full h-[400px] flex items-center justify-center overflow-hidden relative">
