        ...
        {navItems.map((item) => {
          ...
          return (
            <Link 
              key={item.href}
              href={item.href}
              className={`flex items-center space-x-2 px-4 py-2 rounded-md transition-all duration-300 font-manrope text-sm font-semibold
                ${isActive 
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="hidden xl:flex relative group">
          ...
          <input 
            type="text" 
            placeholder="Search..." 
            className="bg-surface/50 border-0 ring-1 ring-outline-variant/20 rounded-md py-1.5 pl-9 pr-4 text-xs font-body w-40 focus:w-60 focus:ring-2 focus:ring-primary transition-all duration-500 outline-none"
          />
        </div>
        ...
        <Link href="/profile" className="w-9 h-9 rounded-full bg-surface-container-high overflow-hidden ring-2 ring-white ambient-shadow cursor-pointer flex items-center justify-center hover:ring-primary/50 transition-all text-on-surface-variant">
          {user && user.photoURL ? (
